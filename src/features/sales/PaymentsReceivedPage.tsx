...
import { usePaymentsReceived } from '@/hooks/queries'
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-heading">Payments Received</h1>
          <p className="text-sm text-muted">Money collected from customers</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-line bg-white px-4 py-3">
          ...
          <p className="num mt-1 text-xl font-semibold text-emerald-600">{formatINR(totalReceived, false)}</p>
                ...
                <tr key={r.voucher_id} className="cursor-pointer hover:bg-canvas" onClick={() => setDrawerVoucherId(r.voucher_id)}>
                  ...
                  <td>
                    <span className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] font-medium ${
                      ...
                      <ArrowDownLeft size={10} />{r.mode}
                    </span>
                  </td>
                  <td className="text-muted text-xs">{r.narration ?? '—'}</td>
                  <td className="r num font-semibold text-emerald-600">{formatINR(Number(r.amount), false)}</td>
                </tr>
              ))}
              {!isLoading && !filtered.length && (
                <tr><td colSpan={6} className="py-10 text-center text-muted">No payments received yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
