-- ============================================================
-- 0020_order_line_guards.sql
-- Prevent integer-overflow from unbounded qty/rate user input.
-- Adds DB-level guards on SO/PO lines AND inside sell/purchase/
-- sales_return/purchase_return RPCs so every entry path is safe.
-- ============================================================

-- ── 1. Clean corrupt SO data (qty > 1 lakh or rate > ₹99,99,999) ──────────
delete from sales_orders where id in (
  select distinct so_id from sales_order_lines
  where qty > 100000 or rate > 9999999900
);
delete from purchase_orders where id in (
  select distinct po_id from purchase_order_lines
  where qty > 100000 or rate > 9999999900
);

-- ── 2. CHECK constraints on order line tables ─────────────────────────────
alter table sales_order_lines
  add constraint sol_qty_positive check (qty > 0),
  add constraint sol_qty_max      check (qty <= 100000),
  add constraint sol_rate_max     check (rate <= 9999999900);

alter table purchase_order_lines
  add constraint pol_qty_positive check (qty > 0),
  add constraint pol_qty_max      check (qty <= 100000),
  add constraint pol_rate_max     check (rate <= 9999999900);

-- ── 3. sell() — add qty/rate guards + fix discount > base ─────────────────
...
begin
  ...
  for it in select * from jsonb_array_elements(p_items) loop
    ...
    if v_rate > 9999999900 then raise exception 'invalid_amount'; end if;
  ...
  -- Discount cannot exceed the pre-GST subtotal
...
-- ── 4. purchase() — add qty/rate guards ───────────────────────────────────
...
-- ── 6. purchase_return() — add qty guards ─────────────────────────────────
...
declare
  ...
  v_cgst bigint := 0; v_sgst bigint := 0; v_igst bigint := 0; v_total bigint;
  v_lines jsonb := '[]'::jsonb; v_stock jsonb := '[]'::jsonb;
  v_dr_acct uuid; v_party_line uuid; v_party_ledger uuid;
  v_org_state text; v_party_state text; v_inter boolean;
begin
  if v_role is null then raise exception 'not_member'; end if;
  if jsonb_array_length(coalesce(p_items,'[]'::jsonb)) = 0 then raise exception 'empty_voucher'; end if;
  ...
  for it in select * from jsonb_array_elements(p_items) loop
    ...
    v_stock := v_stock || jsonb_build_array(jsonb_build_object(
      'stock_item_id', v_item.id, 'qty_change', -v_qty,
      'value_change', -v_line_base, 'reason', 'Purchase Return'));
  end loop;
  ...
  if p_mode = 'credit' then
    select ledger_account_id into v_party_ledger from parties where id = p_party and org_id = p_org;
    if v_party_ledger is null then raise exception 'unknown_party'; end if;
    v_dr_acct := v_party_ledger; v_party_line := p_party;
  else
  ...
  if v_cgst > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_id', sys_account(p_org,'input_cgst'), 'debit', 0, 'credit', v_cgst)); end if;
  if v_sgst > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_id', sys_account(p_org,'input_sgst'), 'debit', 0, 'credit', v_sgst)); end if;
  if v_igst > 0 then v_lines := v_lines || jsonb_build_array(jsonb_build_object('account_id', sys_account(p_org,'input_igst'), 'debit', 0, 'credit', v_igst)); end if;
  v_lines := v_lines || jsonb_build_array(
